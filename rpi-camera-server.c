 ...
 *         and 2019 Brian Webb <webbbn@gmail.com>
 *
 * Based on code from: https://github.com/tjormola/rpi-openmax-demos
 *
...
#define CAM_EXPOSURE_ISO_SENSITIVITY    60
...
//#define USE_UDP
#define MAX_PACKET 32767
//#define MAX_PACKET 50000
#define SEND_PORT 2470
...
static int broken_connection = 0;
...
int main(int argc, char **argv) {
  ...
  if (socketS < 0) {
    die("Error opening the server socket");
  }
  serveraddr.sin_addr.s_addr = htonl(INADDR_ANY);
  if (bind(socketS, (struct sockaddr *)&serveraddr, sizeof(serveraddr)) < 0) {
    die("Failed to bind to network socket");
  }

  // Listen on the TCP port
  if (listen(socketS, 5) < 0) {
    die("Error binding to the TCP listen socket");
  }
#endif
  ...
  // Configure inline headers
  OMX_CONFIG_PORTBOOLEANTYPE inline_headers;
  OMX_INIT_STRUCTURE(inline_headers);
  inline_headers.bEnabled = OMX_TRUE;
  inline_headers.nPortIndex = 201;
  if((r = OMX_SetParameter(ctx.encoder, OMX_IndexParamBrcmVideoAVCInlineHeaderEnable, &inline_headers)) != OMX_ErrorNone) {
    omx_die(r, "Failed to set inline headers for encoder output port 201");
  }
  ...
  int quit_detected = 0, quit_in_keyframe = 0, need_next_buffer_to_be_filled = 1, shutdown = 0;
  ...
  while (!shutdown) {
...
#ifndef USE_UDP
    say("Waiting for a TCP connection\n");
    ...
    sock = accept(socketS, (struct sockaddr *)&clientaddr, &clientlen);
    if (sock < 0) {
      die("Error accepting a TCP connection");
    }
    ...
    if (hostp == NULL) {
      die("Error in gethostbyaddr");
    }

    // Get the hostname of the client
    char *hostaddrp = inet_ntoa(clientaddr.sin_addr);
    if (hostaddrp == NULL) {
      die("Error getting the client hostname");
    }
    say("server established connection with %s (%s)",hostp->h_name, hostaddrp);
    broken_connection = 0;
#endif
...
#else
    uint8_t out_buffer[MAX_FRAME_SIZE];
    uint32_t out_idx = 0;
#endif
    while(1) {
	...
	// Send the data over TCP/UDP
	uint32_t pkt_size = ctx.encoder_ppBuffer_out->nFilledLen;
	uint8_t *data = ctx.encoder_ppBuffer_out->pBuffer + ctx.encoder_ppBuffer_out->nOffset;
#ifdef USE_UDP
	// Break the data into packets if necessary
	while (pkt_size > 0) {
	  uint32_t cur_pkt_size = (pkt_size > MAX_PACKET) ? MAX_PACKET : pkt_size;
	  memcpy(out_buffer, data, cur_pkt_size);
	  size_t output_written = sendto(sock, out_buffer, cur_pkt_size, 0,
					 (const struct sockaddr*)&serveraddr, sizeof(serveraddr));
	  data += cur_pkt_size;
	  pkt_size -= cur_pkt_size;
	  //say("sent: %x %d %d %d %d %d %d %d\n", (ctx.encoder_ppBuffer_out->nFlags & OMX_BUFFERFLAG_SYNCFRAME),
	  //data[0], data[1], data[2], data[3], data[4], cur_pkt_size, output_written);
	}
#else
	// Have we exceeded the maximumbuffer size?
	if ((out_idx + pkt_size) > MAX_FRAME_SIZE) {
	  say("Frame too large\n");
	  out_idx = 0;
	} else {
	  // Append the data into the output buffer
	  memcpy(out_buffer + out_idx, data, pkt_size);
	  out_idx += pkt_size;
	  //say("%d %d %d\n", ctx.encoder_ppBuffer_out->nFlags & OMX_BUFFERFLAG_ENDOFFRAME, pkt_size, out_idx);
	  // Send the data when we've reached the end of frame
	  if (ctx.encoder_ppBuffer_out->nFlags & OMX_BUFFERFLAG_ENDOFFRAME) {
	    size_t output_written = sendto(sock, out_buffer, out_idx, 0,
					   (const struct sockaddr*)&serveraddr, sizeof(serveraddr));
	    if (broken_connection) {
	      say("Broken connection\n");
	      break;
	    } else if (output_written == 0) {
	      say("Warning: Sent 0 byptes to client\n");
	    }
	    //say("sent: %x %d %d %d %d %d %d %d\n", (ctx.encoder_ppBuffer_out->nFlags & OMX_BUFFERFLAG_SYNCFRAME),
	    //data[0], data[1], data[2], data[3], data[4], out_idx, output_written);
	    out_idx = 0;
	  }
	}
#endif
